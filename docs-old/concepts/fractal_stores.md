# Fractal Stores ("Everything is a Store")
...
## Unified Store Model
Every store, regardless of its purpose (e.g., KV, Blob, SQL), consists of two storage areas co-located inside its `state.db` file, managed by a middleware called `lattice-systemstore`:
...
## Context Inheritance
When a node opens a Root Store, the `RecursiveWatcher` service monitors its Intention DAG. If it detects a `ChildStoreDeclaration` intention, it automatically discovers and instantiates the child store.
