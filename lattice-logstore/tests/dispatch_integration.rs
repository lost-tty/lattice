...
async fn test_dispatch_log() {
    ...
    let resp: AppendResponse = invoke_command(&store, "Append", req).await.expect("Append failed");
    ...
    invoke_command::<_, AppendResponse>(&store, "Append", req2).await.expect("Append 2 failed");
    
    // 3. Read All
    let req_read = ReadRequest { tail: None };
    let resp_read: ReadResponse = invoke_command(&store, "Read", req_read).await.expect("Read failed");
    ...
    let resp_tail: ReadResponse = invoke_command(&store, "Read", req_tail).await.expect("Read tail failed");
