...
impl LatticeBackend for RpcBackend {
    ...
    fn store_get_intention(
        ...
        Box::pin(async move {
            ...
            let resp = client
                .store
                .get_intention(GetIntentionRequest {
                    store_id: store_id.as_bytes().to_vec(),
                    hash_prefix: prefix,
                })
                .await;
        ...
        Box::pin(async move {
            ...
            Ok(resp
                ...
                .map(|s| StreamDescriptor {
                    ...
                    param_schema: if s.param_schema.is_empty() {
                        None
                    } else {
                        Some(s.param_schema)
                    },
                    event_schema: if s.event_schema.is_empty() {
                        None
                    } else {
                        Some(s.event_schema)
                    },
