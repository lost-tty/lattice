---
title: "ADR 001: The Fractal Store Model"
---

## Context

Lattice distinguishes between a `Mesh` (container for stores and peers) and a `Store` (single data unit). A `Mesh` owns a `RootStore` which tracks peers and declared sub-stores.
...
We adopt the **Fractal Store Model**:
...
message UniversalOp {
    oneof op {
        bytes app_data = 10;   // Forwarded to inner logic (e.g. KvState)
        SystemOp system = 11;  // Handled by shared StateBackend
    }
}
...
This ADR was implemented. The `Mesh` struct was removed, and `lattice-systemstore` now provides:

- `SystemStore` trait (composed of `SystemReader + SystemWriter + SystemWatcher`)
- `SystemLayer<T>` wrapper that intercepts `SystemOp` during `apply()`
- HeadList CRDT storage in `TABLE_SYSTEM` with `peer/{pubkey}` keys
- `RecursiveWatcher` for automatic child store discovery
- `PeerStrategy` enum (`Independent`, `Inherited`, `Snapshot`)

A `SubstoreManager` trait was not added; hierarchy management goes through `SystemOp::Hierarchy` directly.
