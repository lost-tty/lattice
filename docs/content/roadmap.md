...
- **Epoch Key Encryption**: Shared symmetric key for O(1) gossip payload encryption, enabling efficient peer revocation. On store creation (or membership change), the creator generates a random 32-byte EpochKey and distributes it via an EpochChange message — a one-time O(N) envelope that encrypts the key individually for each authorized peer (X25519 DH derived from Ed25519 keys). All subsequent Intentions encrypt their payload with the shared EpochKey (AEAD: AES-GCM or ChaCha20-Poly1305, ~28 bytes overhead regardless of peer count). On peer revocation, a new epoch is created and the new EpochKey is distributed only to remaining peers — the revoked peer never receives it. Design considerations: nonce partitioning for multi-writer safety (e.g. `author_prefix || random`), epoch rotation triggers (membership change vs periodic), forward secrecy at epoch granularity.
