...
- [ ] **Remove `sync_all` thundering-herd fallback from `handle_missing_dep`**: The 3-tier fallback (fetch_chain → sync_with_peer → sync_all) risks cascading network storms: every time a single peer drops a connection or a packet, the node broadcasts a full Negentropy sync to all online peers. Reduce to 2-tier (fetch_chain → sync_with_peer). If both fail, leave the missing dep for the next event-driven auto-sync cycle to resolve naturally.
- [ ] Review docs/
...
- [x] **CRITICAL — Fix discarded `witness()` result**: `actor.rs:554` does `let _ = store.witness(...)`. If witnessing fails (disk full, redb corruption), the intention is applied to state but never witnessed — silent divergence between state machine and witness log. Must propagate the error or stall the actor.
- [x] **Split `IntentionStoreError::InvalidData(String)`**: Eliminated the catch-all `InvalidData(String)`. Errors are now structured data: `Corruption(Corruption)` with sub-enum `Corruption { FieldLength { table, field, expected, actual }`, `WitnessChainBroken { seq, actual, expected }`, `WitnessDanglingRef { seq, hash } }`, plus `MissingIntention(Hash)` (DAG/chain lookups — recoverable via sync), `ChainWalkFailed { from, ancestor, reason }` (walk_back_until failures), `MalformedInput { field }` (bad API inputs), `Io(std::io::Error)`, `RangeTooLarge(usize)`. All 20 construction sites migrated. Zero `format!` strings in error construction — all fields are typed.
- [x] **Type `LatticeNetError` properly**: 7 of 10 variants are `String`-wrapped. `Sync(String)` alone absorbs `SyncError`, `ReconcileError`, transport failures, and stream failures from 6+ call sites. Replace with typed inner errors: `Sync(#[from] SyncError)`, `Reconcile(#[from] ReconcileError<SyncError>)`, and specific transport/stream variants.
- [x] ~~**Map `IntoStatus` to proper gRPC codes**~~: Added `BackendApiError` enum (`StoreNotFound`, `NotSupported`, `InvalidArgument`, `Validation`, `PermissionDenied`). Replaced bare-string and `io::Error` workarounds in `backend_inprocess.rs`. `IntoStatus` now downcasts `BackendError` to `BackendApiError`, `ExecError`, and `StreamError`, mapping each variant to the correct gRPC status code (`not_found`, `unimplemented`, `invalid_argument`, `failed_precondition`, `permission_denied`, `internal`). Fallback remains `Status::internal`.
- [x] ~~**Type `SystemWatcher::subscribe_events()`**~~: Both the outer `Result` and inner stream `Result<SystemEvent, String>` were always `Ok` — no code path produced errors. Made the entire signature infallible: `fn subscribe_events(&self) -> Pin<Box<dyn Stream<Item = SystemEvent> + Send>>`. Removed `decode_system_event`'s `Result` wrapper, simplified `subscribe_system_events` return type, cleaned up both call sites (`watcher.rs`, `peer_manager.rs`). `PeerManager::start_watching` is now infallible. Removed dead `PeerManagerError::Store(String)` variant.
- [x] ~~**Add `tracing::warn!` to silent error discards**~~: Added `warn!` to all three sites: `peer_manager.rs` `list_acceptable_authors` (peer read failure → empty auth list), `store_manager.rs` `store_ids()`/`list()`/`close_descendants()` (poisoned lock → empty store list), `state_db.rs` `get_meta()` (DB read failure → default meta). All three are intentionally infallible — constrained by trait signatures (`PeerProvider`, `StateMachine`) that serve hot paths or display contexts where `Result` would add complexity without recovery options. Added doc comments to `StateMachine::store_meta()` and `PeerProvider::list_acceptable_authors()` explaining the design rationale.
- [x] ~~**Clean up `StoreManagerError` junk-drawer variants**~~: Eliminated `Store(String)`, `Registry(String)`, and `Lock(String)`. Replaced with typed variants: `NotSystemStore` (4 sites), `Unauthorized { peer, reason }` (1 site), `PeerManager(#[from] PeerManagerError)` (2 sites — revoke + activate), `Storage(#[from] StateError)` (2 sites — peek_store_info + get_or_open), `LockPoisoned` (5 sites). `Registry(self.register().to_string())` self-flattening replaced with direct `?`. Also removed 2 in-function `use lattice_model::InviteStatus` statements.
- [x] ~~**Reduce `StateDbError` variant count**~~: Created `RedbError` wrapper enum for the 5 redb error types (`Database`, `Table`, `Transaction`, `Storage`, `Commit`). `StateDbError` collapsed from 12 variants to 8 with a single `Redb(#[from] RedbError)`. `SystemReadError` collapsed from 6 variants to 3 with the same pattern. Added transitive `From` impls so `?` on individual redb errors still works. `KvTableResultExt` updated (`Storage(e)` → `Redb(e.into())`).
