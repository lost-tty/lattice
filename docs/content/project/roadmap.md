...
| **M7**           | Client/Daemon split: library extraction, `lattice` daemon with UDS gRPC, `LatticeBackend` trait, RPC event streaming, socket security        |
...
- [ ] **Batch index updates during bulk ingest**: Derived indexes are updated per `witness()` call, causing redundant work during bulk operations (bootstrap, sync catch-up). Two main cases: (1) `TABLE_FRONTIER` — same parent/dep frontiers decoded+encoded repeatedly when ingesting a batch that shares ancestors; accumulate in-memory, flush at end of txn. (2) `TABLE_AUTHOR_TIPS` — every intention by the same author overwrites the previous tip; only the last write per author per batch matters. Only worth doing if bulk ingest profiling shows these as hot spots.
...
- [ ] **Consolidate `PeerStatus`**: enum is defined twice — `lattice-model::PeerStatus` (Rust domain) and `lattice_proto::storage::PeerStatus` (proto). Hand-written conversions in `lattice-systemstore`, no alignment test. API proto still carries status as `string` because its proto tree can't import `lattice-proto`. Plan: flip dep direction (lattice-model → lattice-proto), delete lattice-model's enum, everyone uses `lattice_proto::storage::PeerStatus`. API wire stays string via a single boundary helper. Same for `ChildStatus` and `InviteStatus`.
