...
async fn test_watch_gap_race() {
    ...
    let collect_future = async {
        while let Some(res) = stream.next().await {
            ...
            if received_count >= 50 { break; } // Just verify we get a chunk of them
        }
    };
    ...
    assert!(received_count > 0, "Stream missed all events during race condition");
}
...
async fn test_watch_invalid_regex() {
    ...
    assert!(msg.to_lowercase().contains("regex") || msg.contains("parse error") || msg.contains("invalid"), 
            "Error should mention regex matching failure, got: {}", msg);
}
...
async fn test_malformed_replication_entry() {
    ...
    store.writer.entry_tx().send(garbage).expect("failed to inject garbage");
...
async fn test_watch_complex_regex() {
    ...
    store.put(b"user/alice".to_vec(), b"v1".to_vec()).await.unwrap();
    store.put(b"user/123".to_vec(), b"v2".to_vec()).await.unwrap(); // Should not match
    store.put(b"admin/alice".to_vec(), b"v3".to_vec()).await.unwrap(); // Should not match
    store.put(b"user/bob".to_vec(), b"v4".to_vec()).await.unwrap();
