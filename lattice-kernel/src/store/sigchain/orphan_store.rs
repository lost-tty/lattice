...
impl OrphanStore {
    ...
    pub fn insert(
        ...
        let key = OrphanKey::new(
            PubKey::from(*author),
            Hash::from(*prev_hash),
            Hash::from(*entry_hash),
        );
...
pub(crate) mod tests {
    ...
    fn test_orphan_store_basic() {
        ...
        let fake_tip = crate::entry::ChainTip {
            seq: 4,
            hash: prev_hash,
            hlc: HLC::default(),
        };
        ...
        let found = store
            .find_by_prev_hash(&PubKey::from(author), &Hash::from(prev_hash))
            .unwrap();
        ...
        // Delete
        store
            .delete(&PubKey::from(author), &Hash::from(prev_hash), &entry_hash)
            .unwrap();
