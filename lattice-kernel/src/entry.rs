...
use lattice_proto::storage::{
    ChainTip as ProtoChainTip, Entry as ProtoEntry, SignedEntry as ProtoSignedEntry,
};

use ed25519_dalek::{Signature, VerifyingKey};
use lattice_model::types::{Hash, PubKey, Signature as Sig};
use lattice_model::{NodeError, NodeIdentity};
use prost::Message;
use thiserror::Error;
    ...
    pub fn next_after(tip: Option<&ChainTip>) -> EntryBuilder {
        match tip {
            Some(t) => {
                ...
                EntryBuilder::new(t.seq + 1, timestamp).prev_hash(t.hash)
            }
            None => EntryBuilder::new(1, HLC::now()),
        }
    }
...
impl SignedEntry {
    ...
    pub fn verify(&self) -> Result<(), EntryError> {
        ...
        let public_key =
            VerifyingKey::from_bytes(&pk_bytes).map_err(|_| NodeError::InvalidSignature)?;
...
impl From<Entry> for ProtoEntry {
    fn from(e: Entry) -> Self {
        ProtoEntry {
            ...
            causal_deps: e.causal_deps.iter().map(|h| h.to_vec()).collect(),
            seq: e.seq,
    ...
    fn try_from(p: ProtoEntry) -> Result<Self, Self::Error> {
        ...
        Ok(Entry {
            ...
            timestamp: p
                .timestamp
                .map(Into::into)
                .ok_or(EntryError::MissingTimestamp)?,
            payload: p.payload,
        })
    }
}
...
impl TryFrom<ProtoSignedEntry> for SignedEntry {
    ...
    fn try_from(p: ProtoSignedEntry) -> Result<Self, Self::Error> {
        ...
        let public_key =
            VerifyingKey::from_bytes(&author_id).map_err(|_| NodeError::InvalidSignature)?;
...
impl EntryBuilder {
    ...
    pub fn causal_deps(mut self, hashes: Vec<Hash>) -> Self {
        self.entry.causal_deps = hashes;
        self
    }
...
// --- LogEntry and ChainTipInfo trait implementations ---

impl lattice_model::LogEntry for SignedEntry {
    fn hash(&self) -> Hash {
        self.hash()
    }
...
impl lattice_model::ChainTipInfo for ChainTip {
    fn seq(&self) -> u64 {
        self.seq
    }
