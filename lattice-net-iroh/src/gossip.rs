...
use lattice_net_types::{GossipLayer, GossipError, NetworkEvent};
...
impl GossipManager {
    pub async fn new(
        endpoint: &crate::IrohTransport,
    ) -> Result<Self, GossipError> {
        let gossip = Gossip::builder()
            .spawn(endpoint.endpoint().clone());
...
impl GossipLayer for GossipManager {
    ...
    async fn join_peers(&self, store_id: uuid::Uuid, peers: Vec<PubKey>) -> Result<(), GossipError> {
        let senders = self.senders.read().await;
        if let Some(sender) = senders.get(&store_id) {
            let iroh_peers: Vec<iroh::PublicKey> = peers.iter()
                .filter_map(|p| iroh::PublicKey::from_bytes(&p.as_bytes()).ok())
                .collect();
            if !iroh_peers.is_empty() {
                sender.join_peers(iroh_peers)
                    .await
                    .map_err(|e| GossipError::Subscribe(e.to_string()))?;
            }
        }
