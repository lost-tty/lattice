...
fn build_node(
    ...
    Arc<Node>,
    tokio::sync::broadcast::Receiver<lattice_model::NetEvent>,
    tokio::sync::broadcast::Sender<lattice_model::NetEvent>,
) {
...
async fn test_iroh_backend_starts_successfully() {
    ...
    let service = lattice_net::network::NetworkService::new(node.clone(), backend, net_rx);
...
async fn test_gossip_actor_survives_backend_creation() {
    ...
    let gossip = std::sync::Arc::new(
        lattice_net_iroh::GossipManager::new(&transport)
            .await
            .expect("gossip"),
    );
...
async fn test_two_iroh_nodes_store_sync() {
    ...
    let service_a = lattice_net::network::NetworkService::new(node_a.clone(), backend_a, net_rx_a);
    ...
    let service_b = lattice_net::network::NetworkService::new(node_b.clone(), backend_b, net_rx_b);
    ...
    assert!(
        store_b.is_some(),
        "Node B should have the store after joining"
    );
