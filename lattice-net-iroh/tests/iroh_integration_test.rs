...
async fn test_iroh_backend_starts_successfully() {
    ...
    let backend = IrohBackend::new(node.signing_key().clone(), node.clone())
        .await
...
async fn test_gossip_actor_survives_backend_creation() {
    ...
    let transport = lattice_net_iroh::IrohTransport::new(node.signing_key().clone())
        .await
        .expect("transport");
...
async fn test_two_iroh_nodes_store_sync() {
    ...
    let backend_a = IrohBackend::new(node_a.signing_key().clone(), node_a.clone())
        .await
    ...
    let backend_b = IrohBackend::new(node_b.signing_key().clone(), node_b.clone())
        .await
