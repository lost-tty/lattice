...
async fn test_broadcast_gossip_bidirectional() {
    ...
    assert!(
        received_at_b,
        "B should receive A's entry via gossip (A→B direction)"
    );
    ...
    assert!(
        received_at_a,
        "A should receive B's entry via gossip (B→A direction)"
    );
}
...
async fn test_dynamic_peer_joining_via_gossip() {
    ...
    assert!(
        received_at_b,
        "B should receive A's entry via gossip after dynamic join"
    );
