...
async fn test_chain_fetch_linear_gap() {
    ...
    let result = server_b.fetch_chain(store_b.id(), node_a.node_id(), target, Some(since)).await;
...
async fn test_chain_fetch_invalid_since() {
    ...
    let result = server_b.fetch_chain(store_b.id(), node_a.node_id(), target, None).await;
...
async fn test_chain_fetch_fork_detect() {
    ...
    let result = server_b.fetch_chain(store_b.id(), node_a.node_id(), target, Some(fake_since)).await;
