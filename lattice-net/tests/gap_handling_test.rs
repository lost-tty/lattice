...
async fn test_unresponsive_peer_fallback() {
    ...
    let store_c_handle = common::join_store_via_event(&node_c, a_pubkey, store_id, invite_c.secret)
        .await
        .unwrap();
    ...
    // Now setup B
    let store_b_handle = common::join_store_via_event(&node_b, a_pubkey, store_id, invite_b.secret)
        .await
        .unwrap();
    ...
    server_b
        .handle_missing_dep(store_id, missing, Some(a_pubkey))
        .await
