...
async fn test_peer_persistence_after_bootstrap() {
    ...
    let count = server_b
        .bootstrap_from_peer(store_id, a_pubkey)
        .await
        .expect("bootstrap");
    ...
    while let Some(res) = stream.next().await {
        ...
        let content =
            lattice_kernel::proto::weaver::WitnessContent::decode(entry.content.as_slice())
                .expect("decode witness content");
        let intent_hash =
            lattice_model::types::Hash::try_from(content.intention_hash).expect("invalid hash");

        tracing::info!("Entry: seq={}, hash={}", entry.seq, intent_hash);

        let ints = net_store
            .fetch_intentions(vec![intent_hash])
            .await
            .expect("fetch");
    ...
    if let Some(p) = peer_a {
        ...
        assert_eq!(
            p.status,
            PeerStatus::Active,
            "Peer A should be Active in B's store"
        );
    } else {
