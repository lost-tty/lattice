...
async fn test_sync_preserves_causal_order_in_witness_log() {
    ...
    let hash1 = WitnessContent::decode(log_a[len_a - 3].content.as_slice())
        .unwrap()
        .intention_hash;
    let hash2 = WitnessContent::decode(log_a[len_a - 2].content.as_slice())
        .unwrap()
        .intention_hash;
    let hash3 = WitnessContent::decode(log_a[len_a - 1].content.as_slice())
        .unwrap()
        .intention_hash;
