...
use crate::proto::{
    ...
    StreamList, StreamDescriptor, SubscribeRequest, StoreEvent,
};
...
impl DynamicStoreService for DynamicStoreServiceImpl {
    async fn exec(&self, request: Request<ExecRequest>) -> Result<Response<ExecResponse>, Status> {
        ...
        match self.backend.store_exec(store_id, &req.method, &req.payload).await {
            ...
            Err(e) => {
                ...
                let error_code = if error.contains("not found") {
                    if error.contains("Store") { ErrorCode::StoreNotFound }
                    else if error.contains("Method") { ErrorCode::MethodNotFound }
                    else { ErrorCode::ExecutionFailed }
    ...
    async fn get_descriptor(&self, request: Request<StoreId>) -> Result<Response<DescriptorResponse>, Status> {
        ...
        self.backend.store_get_descriptor(store_id).await
            .map(|(file_descriptor_set, service_name)| Response::new(DescriptorResponse { file_descriptor_set, service_name }))
            .map_err(|e| Status::internal(e.to_string()))
    }

    async fn list_methods(&self, request: Request<StoreId>) -> Result<Response<MethodList>, Status> {
        let store_id = Self::parse_uuid(&request.into_inner().id)?;
        self.backend.store_list_methods(store_id).await
            .map(|methods| {
                let methods = methods.into_iter().map(|(name, description)| MethodInfo { name, description }).collect();
                Response::new(MethodList { methods })
            })
            .map_err(|e| Status::internal(e.to_string()))
    }

    async fn list_streams(&self, request: Request<StoreId>) -> Result<Response<StreamList>, Status> {
        let store_id = Self::parse_uuid(&request.into_inner().id)?;
        self.backend.store_list_streams(store_id).await
            .map(|streams| {
                let streams = streams.into_iter().map(|s| StreamDescriptor {
                    name: s.name,
