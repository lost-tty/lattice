    ...
    pub fn join(&self, peer_id: PubKey, store_id: Uuid, secret: Vec<u8>) -> Result<(), NodeError> {
        ...
        if self.store_manager.get_handle(&store_id).is_some() {
            return Err(NodeError::Validation(format!(
                "Already a member of mesh/store {}",
                store_id
            )));
        }
        ...
        // Check if join is already in progress
        {
            ...
            if pending.contains(&store_id) {
                return Err(NodeError::Validation(format!(
                    "Join for mesh {} is already in progress",
                    store_id
                )));
            }
    ...
    fn emit_user_event(&self, event: UserEvent) {
        match event {
            UserEvent::JoinFailed { store_id, reason } => {
                let _ = self
                    .event_tx
                    .send(NodeEvent::JoinFailed { store_id, reason });
            }
            UserEvent::SyncResult {
                store_id,
                peers_synced,
                entries_sent,
                entries_received,
            } => {
                let _ = self.event_tx.send(NodeEvent::SyncResult {
                    store_id,
                    peers_synced,
                    entries_sent,
                    entries_received,
...
impl NodeProviderAsync for Node {
    ...
    async fn accept_join(
        ...
        let acceptance = self
            .accept_join(peer_pubkey, store_id, invite_secret)
            .await
            .map_err(|e| NodeProviderError::Join(e.to_string()))?;
...
use lattice_net_types::{NetworkStoreRegistry, NodeProviderExt};
...
impl NodeProviderExt for Node {
    ...
    fn get_peer_provider(&self, store_id: &Uuid) -> Option<std::sync::Arc<dyn PeerProvider>> {
        self.store_manager
            .get_peer_manager(store_id)
            .map(|pm| pm as std::sync::Arc<dyn PeerProvider>)
    }
}
...
mod tests {
    ...
    async fn test_create_multiple_meshes() {
        ...
        assert!(node
            ...
            .expect("list")
            .iter()
            .any(|(id, _)| *id == store_id_1));
        assert!(node
            .meta()
            .list_rootstores()
            .expect("list")
            .iter()
            .any(|(id, _)| *id == store_id_2));
    ...
    async fn test_mesh_persists_across_sessions() {
        ...
        assert!(
            meshes.iter().any(|(id, _)| *id == store_id),
            "Mesh should persist"
        );
    ...
    async fn test_set_name_updates_store() {
        ...
        assert_ne!(
            ...
            "New name should differ from initial"
        );
    ...
    async fn test_create_invite_token() {
        ...
        let token = node
            .store_manager()
            .create_invite(store_id, node.node_id())
            .await
    ...
    async fn test_accept_join() {
        ...
        let acceptance = node
            .accept_join(peer_pubkey, store_id, &invite.secret)
            .await
            .expect("accept_join");
    ...
    async fn test_invite_join_sync_flow() {
        ...
        let node_b = std::sync::Arc::new(
            test_node_builder(data_dir_b.clone())
                .build()
        ...
        assert_eq!(
            invite.inviter,
            node_a.node_id(),
            "Token should contain A's pubkey"
        );
        ...
        let acceptance = node_a
            .accept_join(node_b.node_id(), invite.store_id, &invite.secret)
            .await
            .expect("accept join");
