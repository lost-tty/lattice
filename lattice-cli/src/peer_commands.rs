...
pub async fn cmd_peer_list(backend: &dyn LatticeBackend, store_id: Option<Uuid>, writer: Writer) -> CmdResult {
...
pub async fn cmd_peer_revoke(backend: &dyn LatticeBackend, store_id: Option<Uuid>, pubkey: &str, writer: Writer) -> CmdResult {
    let mut w = writer.clone();
...
pub async fn cmd_peer_invite(backend: &dyn LatticeBackend, store_id: Option<Uuid>, writer: Writer) -> CmdResult {
    use owo_colors::OwoColorize;
    let mut w = writer.clone();
